...
public final class Matcher78 implements Matcher {
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher79.INSTANCE.match(
            node.getChild(Matcher78.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher80.INSTANCE.match(
            node.getChild(Matcher78.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher81.INSTANCE.match(
            node.getChild(Matcher78.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
