...
public final class Matcher238 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher238.EXPECTED_TYPE)
            && node.getChildCount() == Matcher238.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher238.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher238.FIRST_CHILD_ID))
            );
            children.put(
                Matcher238.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher238.SECOND_CHILD_ID))
            );
        }
        return result;
    }
