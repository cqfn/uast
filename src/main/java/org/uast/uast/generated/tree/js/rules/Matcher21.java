...
public final class Matcher21 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher21.EXPECTED_TYPE)
            && node.getChildCount() == Matcher21.EXPECTED_COUNT
            && Matcher21.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher22.INSTANCE.match(
            node.getChild(Matcher21.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher23.INSTANCE.match(
            node.getChild(Matcher21.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher24.INSTANCE.match(
            node.getChild(Matcher21.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
