...
public final class Matcher94 implements Matcher {
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher95.INSTANCE.match(
            node.getChild(Matcher94.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher96.INSTANCE.match(
            node.getChild(Matcher94.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
