...
public final class Matcher36 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher36.EXPECTED_TYPE)
            && node.getChildCount() == Matcher36.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher36.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher36.FIRST_CHILD_ID))
            );
        }
        return result;
    }
