...
public final class Matcher48 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher48.EXPECTED_TYPE)
            && node.getChildCount() == Matcher48.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher48.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher48.FIRST_CHILD_ID))
            );
        }
        return result;
    }
