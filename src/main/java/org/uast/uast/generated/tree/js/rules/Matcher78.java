...
public final class Matcher78 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher78.EXPECTED_TYPE)
            && node.getChildCount() == Matcher78.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher78.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher78.FIRST_CHILD_ID))
            );
        }
        return result;
    }
