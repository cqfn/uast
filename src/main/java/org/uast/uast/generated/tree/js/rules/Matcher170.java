...
public final class Matcher170 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher170.EXPECTED_TYPE)
            && node.getChildCount() == Matcher170.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher170.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher170.FIRST_CHILD_ID))
            );
        }
        return result;
    }
