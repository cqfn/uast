...
public final class Matcher256 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        if (result) {
            ...
            children.put(
                Matcher256.FIFTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher256.FIFTH_CHILD_ID))
            );
            children.put(
                Matcher256.SIXTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher256.SIXTH_CHILD_ID))
            );
            children.put(
                Matcher256.SEVENTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher256.SEVENTH_CHILD_ID))
            );
        }
        return result;
    }
