...
public final class Rule138 implements Converter {
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        if (matched) {
            result = Rule138.firstBuilder(factory, children, data);
        }
        return result;
    }
    ...
    private static Node secondBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule138.MODIFIER_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule138.thirdBuilder(factory, data));
        list.add(Rule138.fourthBuilder(factory, data));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node thirdBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule138.MODIFIER);
    ...
    private static Node fifthBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule138.PARAMETER_BLOCK);
