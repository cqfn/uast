 ...
 * @since 1.0
 */
public final class Rule134 implements Converter {
    /**
     * The instance.
     */
    public static final Converter INSTANCE = new Rule134();
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        if (matched) {
            result = Rule134.firstBuilder(factory, children, data);
        }
        return result;
    }
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        list.add(Rule134.secondBuilder(factory, data));
        list.addAll(children.get(Rule134.FIRST_HOLE_ID));
        list.add(Rule134.fourthBuilder(factory, children));
        list.add(Rule134.seventhBuilder(factory));
    ...
    private static Node secondBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule134.MODIFIER_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule134.thirdBuilder(factory, data));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node thirdBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule134.MODIFIER);
    ...
    private static Node fourthBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule134.IMPLEMENTS_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule134.fifthBuilder(factory, children));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node fifthBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule134.CLASS_TYPE);
        final List<Node> list = new LinkedList<>();
        list.add(Rule134.sixthBuilder(factory, children));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node sixthBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule134.NAME);
