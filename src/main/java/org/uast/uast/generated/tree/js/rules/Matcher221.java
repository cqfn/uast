...
public final class Matcher221 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher221.EXPECTED_TYPE)
            && node.getChildCount() == Matcher221.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher221.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher221.FIRST_CHILD_ID))
            );
        }
        return result;
    }
