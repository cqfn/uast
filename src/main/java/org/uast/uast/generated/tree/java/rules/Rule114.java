 ...
 * @since 1.0
 */
public final class Rule114 implements Converter {
    /**
     * The instance.
     */
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        if (matched) {
            result = Rule114.firstBuilder(factory, children, data);
        }
        return result;
    }
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        list.add(Rule114.secondBuilder(factory, data));
        list.addAll(children.get(Rule114.FIRST_HOLE_ID));
        list.addAll(children.get(Rule114.SECOND_HOLE_ID));
        list.add(Rule114.fifthBuilder(factory, children));
        list.addAll(children.get(Rule114.THIRD_HOLE_ID));
    ...
    private static Node secondBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule114.MODIFIER_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule114.thirdBuilder(factory, data));
        list.add(Rule114.fourthBuilder(factory, data));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node thirdBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule114.MODIFIER);
