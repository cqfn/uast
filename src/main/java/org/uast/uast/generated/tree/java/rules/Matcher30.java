...
public final class Matcher30 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher30.EXPECTED_TYPE)
            ...
            && Matcher30.EXPECTED_DATA.equals(node.getData())
            && Matcher30.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher31.INSTANCE.match(
            node.getChild(Matcher30.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher32.INSTANCE.match(
            node.getChild(Matcher30.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
