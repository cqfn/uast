...
public final class Matcher18 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher18.EXPECTED_TYPE)
            && node.getChildCount() == Matcher18.EXPECTED_COUNT
            && Matcher18.EXPECTED_DATA.equals(node.getData())
            && Matcher18.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher19.INSTANCE.match(
            node.getChild(Matcher18.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher20.INSTANCE.match(
            node.getChild(Matcher18.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
