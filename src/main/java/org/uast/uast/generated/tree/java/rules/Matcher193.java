...
public final class Matcher193 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher193.EXPECTED_TYPE)
            ...
            && Matcher195.INSTANCE.match(node.getChild(1), children, data);
        if (result) {
            children.put(
                Matcher193.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher193.FIRST_CHILD_ID))
            );
            children.put(
                Matcher193.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher193.SECOND_CHILD_ID))
            );
            children.put(
                Matcher193.THIRD_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher193.THIRD_CHILD_ID))
            );
            children.put(
                Matcher193.FOURTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher193.FOURTH_CHILD_ID))
            );
        }
        return result;
    }
