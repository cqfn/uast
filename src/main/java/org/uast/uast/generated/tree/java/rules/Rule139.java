...
public final class Rule139 implements Converter {
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        if (matched) {
            result = Rule139.firstBuilder(factory, children, data);
        }
        return result;
    }
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule139.FUNCTION_DECLARA);
        final List<Node> list = new LinkedList<>();
        list.add(Rule139.secondBuilder(factory, data));
        list.addAll(children.get(Rule139.FIRST_HOLE_ID));
        list.addAll(children.get(Rule139.SECOND_HOLE_ID));
        list.add(Rule139.fifthBuilder(factory, children));
    ...
    private static Node secondBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule139.MODIFIER_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule139.thirdBuilder(factory, data));
        list.add(Rule139.fourthBuilder(factory, data));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node thirdBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule139.MODIFIER);
