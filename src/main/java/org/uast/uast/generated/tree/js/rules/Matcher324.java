...
public final class Matcher324 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher324.EXPECTED_TYPE)
            && node.getChildCount() == Matcher324.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher324.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher324.FIRST_CHILD_ID))
            );
        }
        return result;
    }
