...
public final class Matcher33 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher33.EXPECTED_TYPE)
            && node.getChildCount() == Matcher33.EXPECTED_COUNT
            && Matcher33.EXPECTED_DATA.equals(node.getData());
        if (result) {
            children.put(
                Matcher33.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher33.FIRST_CHILD_ID))
            );
            children.put(
                Matcher33.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher33.SECOND_CHILD_ID))
            );
        }
        return result;
    }
