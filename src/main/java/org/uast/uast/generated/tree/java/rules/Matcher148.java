...
public final class Matcher148 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher148.EXPECTED_TYPE)
            ...
            && Matcher148.EXPECTED_DATA.equals(node.getData());
        if (result) {
            children.put(
                Matcher148.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher148.FIRST_CHILD_ID))
            );
            children.put(
                Matcher148.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher148.SECOND_CHILD_ID))
            );
        }
        return result;
    }
