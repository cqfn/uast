...
public final class Matcher34 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher34.EXPECTED_TYPE)
            && node.getChildCount() == Matcher34.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher34.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher34.FIRST_CHILD_ID))
            );
        }
        return result;
    }
