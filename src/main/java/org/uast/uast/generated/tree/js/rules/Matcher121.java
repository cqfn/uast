...
public final class Matcher121 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher121.EXPECTED_TYPE)
            && node.getChildCount() == Matcher121.EXPECTED_COUNT
            && Matcher121.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher122.INSTANCE.match(
            node.getChild(Matcher121.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher123.INSTANCE.match(
            node.getChild(Matcher121.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher124.INSTANCE.match(
            node.getChild(Matcher121.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
