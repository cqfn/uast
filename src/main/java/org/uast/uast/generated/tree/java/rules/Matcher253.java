...
public final class Matcher253 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher253.EXPECTED_TYPE)
            ...
            && Matcher254.INSTANCE.match(node.getChild(0), children, data);
        if (result) {
            children.put(
            ...
            children.put(
                Matcher253.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher253.SECOND_CHILD_ID))
            );
            children.put(
                Matcher253.THIRD_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher253.THIRD_CHILD_ID))
            );
            children.put(
                Matcher253.FOURTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher253.FOURTH_CHILD_ID))
            );
            children.put(
                Matcher253.FIFTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher253.FIFTH_CHILD_ID))
            );
            children.put(
                Matcher253.SIXTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher253.SIXTH_CHILD_ID))
            );
        }
        return result;
    }
