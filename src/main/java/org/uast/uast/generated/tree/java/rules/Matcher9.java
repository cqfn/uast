...
public final class Matcher9 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher9.EXPECTED_TYPE)
            ...
            && Matcher9.EXPECTED_DATA.equals(node.getData());
        if (result) {
            children.put(
                Matcher9.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher9.FIRST_CHILD_ID))
            );
            children.put(
                Matcher9.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher9.SECOND_CHILD_ID))
            );
        }
        return result;
    }
