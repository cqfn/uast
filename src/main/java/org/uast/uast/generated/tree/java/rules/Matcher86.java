...
public final class Matcher86 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher86.EXPECTED_TYPE)
            && node.getChildCount() == Matcher86.EXPECTED_COUNT
            && Matcher86.EXPECTED_DATA.equals(node.getData())
            && Matcher86.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher87.INSTANCE.match(
            node.getChild(Matcher86.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher88.INSTANCE.match(
            node.getChild(Matcher86.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
