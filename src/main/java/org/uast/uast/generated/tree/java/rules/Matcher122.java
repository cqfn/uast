...
public final class Matcher122 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher122.EXPECTED_TYPE)
            ...
            && Matcher122.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher123.INSTANCE.match(
            node.getChild(Matcher122.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher124.INSTANCE.match(
            node.getChild(Matcher122.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
