...
public final class Matcher22 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher22.EXPECTED_TYPE)
            && node.getChildCount() == Matcher22.EXPECTED_COUNT
            && Matcher22.EXPECTED_DATA.equals(node.getData())
            && Matcher22.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher23.INSTANCE.match(
            node.getChild(Matcher22.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher24.INSTANCE.match(
            node.getChild(Matcher22.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
