...
public final class Matcher194 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher194.EXPECTED_TYPE)
            ...
            && Matcher196.INSTANCE.match(node.getChild(1), children, data);
        if (result) {
            children.put(
                Matcher194.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher194.FIRST_CHILD_ID))
            );
            children.put(
                Matcher194.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher194.SECOND_CHILD_ID))
            );
            children.put(
                Matcher194.THIRD_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher194.THIRD_CHILD_ID))
            );
            children.put(
                Matcher194.FOURTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher194.FOURTH_CHILD_ID))
            );
        }
        return result;
    }
