...
public final class Matcher366 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher366.EXPECTED_TYPE)
            && node.getChildCount() == Matcher366.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher366.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher366.FIRST_CHILD_ID))
            );
        }
        return result;
    }
