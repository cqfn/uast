...
public final class Matcher418 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher418.EXPECTED_TYPE)
            && node.getChildCount() == Matcher418.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher418.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher418.FIRST_CHILD_ID))
            );
        }
        return result;
    }
