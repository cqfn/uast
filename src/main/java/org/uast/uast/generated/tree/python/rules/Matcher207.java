...
public final class Matcher207 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher207.EXPECTED_TYPE)
            && node.getChildCount() == Matcher207.EXPECTED_COUNT
            && Matcher207.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher208.INSTANCE.match(
            node.getChild(Matcher207.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher209.INSTANCE.match(
            node.getChild(Matcher207.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher211.INSTANCE.match(
            node.getChild(Matcher207.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
