...
public final class Matcher122 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher122.EXPECTED_TYPE)
            && node.getChildCount() == Matcher122.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher122.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher122.FIRST_CHILD_ID))
            );
        }
        return result;
    }
