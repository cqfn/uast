...
public final class Matcher252 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher252.EXPECTED_TYPE)
            && node.getChildCount() == Matcher252.EXPECTED_COUNT
            && Matcher252.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher253.INSTANCE.match(
            node.getChild(Matcher252.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher254.INSTANCE.match(
            node.getChild(Matcher252.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher256.INSTANCE.match(
            node.getChild(Matcher252.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
