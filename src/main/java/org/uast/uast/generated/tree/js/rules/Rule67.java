...
public final class Rule67 implements Converter {
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        list.add(Rule67.fourthBuilder(factory, children));
    ...
    private static Node secondBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule67.VARIABLE);
        final List<Node> list = new LinkedList<>();
        list.add(Rule67.thirdBuilder(factory, children));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node thirdBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule67.NAME);
    ...
    private static Node fourthBuilder(final Factory factory,
        ...
        final Builder builder = factory.createBuilder(Rule67.VARIABLE);
        final List<Node> list = new LinkedList<>();
        list.add(Rule67.fifthBuilder(factory, children));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
