...
public final class Rule122 implements Converter {
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        final boolean matched = Matcher361.INSTANCE.match(node, children, data);
        if (matched) {
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule122.PROGRAM);
        final List<Node> list = new LinkedList<>();
        list.addAll(children.get(Rule122.FIRST_HOLE_ID));
        list.addAll(children.get(Rule122.SECOND_HOLE_ID));
