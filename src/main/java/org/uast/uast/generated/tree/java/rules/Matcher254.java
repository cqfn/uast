...
public final class Matcher254 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher254.EXPECTED_TYPE)
            ...
            && Matcher255.INSTANCE.match(node.getChild(0), children, data);
        if (result) {
            children.put(
                Matcher254.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher254.FIRST_CHILD_ID))
            );
            children.put(
                Matcher254.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher254.SECOND_CHILD_ID))
            );
            children.put(
            ...
            children.put(
                Matcher254.SEVENTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher254.SEVENTH_CHILD_ID))
            );
        }
        return result;
    }
