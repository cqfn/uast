...
public final class Matcher277 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher277.EXPECTED_TYPE)
            && node.getChildCount() == Matcher277.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher277.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher277.FIRST_CHILD_ID))
            );
        }
        return result;
    }
