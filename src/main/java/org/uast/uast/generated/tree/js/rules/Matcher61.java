...
public final class Matcher61 implements Matcher {
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher62.INSTANCE.match(
            node.getChild(Matcher61.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher63.INSTANCE.match(
            node.getChild(Matcher61.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher64.INSTANCE.match(
            node.getChild(Matcher61.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
