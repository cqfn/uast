...
public final class Rule136 implements Converter {
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        final boolean matched = Matcher411.INSTANCE.match(node, children, data);
        if (matched) {
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule136.FUNCTION_DECLARA);
        final List<Node> list = new LinkedList<>();
        list.addAll(children.get(Rule136.FIRST_HOLE_ID));
        list.addAll(children.get(Rule136.SECOND_HOLE_ID));
        list.addAll(children.get(Rule136.THIRD_HOLE_ID));
