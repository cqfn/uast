 ...
 * @since 1.0
 */
public final class Rule148 implements Converter {
    /**
     * The instance.
     */
    public static final Converter INSTANCE = new Rule148();
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        final boolean matched = Matcher262.INSTANCE.match(node, children, data);
        if (matched) {
            result = Rule148.firstBuilder(factory, children, data);
        }
        return result;
    }
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule148.FUNCTION_DECLARA);
        final List<Node> list = new LinkedList<>();
        list.add(Rule148.secondBuilder(factory, data));
        list.addAll(children.get(Rule148.FIRST_HOLE_ID));
        list.addAll(children.get(Rule148.SECOND_HOLE_ID));
        list.add(Rule148.fourthBuilder(factory, children));
        list.addAll(children.get(Rule148.SEVENTH_HOLE_ID));
    ...
    private static Node secondBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule148.MODIFIER_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule148.thirdBuilder(factory, data));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node thirdBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule148.MODIFIER);
    ...
    private static Node fourthBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule148.PARAMETER_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.addAll(children.get(Rule148.THIRD_HOLE_ID));
        list.addAll(children.get(Rule148.FOURTH_HOLE_ID));
        list.addAll(children.get(Rule148.FIFTH_HOLE_ID));
        list.addAll(children.get(Rule148.SIXTH_HOLE_ID));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
