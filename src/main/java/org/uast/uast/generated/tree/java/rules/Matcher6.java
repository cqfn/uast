...
public final class Matcher6 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher6.EXPECTED_TYPE)
            ...
            && Matcher6.EXPECTED_DATA.equals(node.getData())
            && Matcher6.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher7.INSTANCE.match(
            node.getChild(Matcher6.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher8.INSTANCE.match(
            node.getChild(Matcher6.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
