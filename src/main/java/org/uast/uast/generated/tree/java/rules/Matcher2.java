...
public final class Matcher2 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher2.EXPECTED_TYPE)
            ...
            && Matcher2.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher3.INSTANCE.match(
            node.getChild(Matcher2.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher4.INSTANCE.match(
            node.getChild(Matcher2.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
