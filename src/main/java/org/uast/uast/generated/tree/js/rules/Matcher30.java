...
public final class Matcher30 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher30.EXPECTED_TYPE)
            && node.getChildCount() == Matcher30.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher30.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher30.FIRST_CHILD_ID))
            );
        }
        return result;
    }
