...
public final class Matcher87 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher87.EXPECTED_TYPE)
            && node.getChildCount() == Matcher87.EXPECTED_COUNT
            && Matcher87.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher88.INSTANCE.match(
            node.getChild(Matcher87.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher89.INSTANCE.match(
            node.getChild(Matcher87.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher90.INSTANCE.match(
            node.getChild(Matcher87.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
