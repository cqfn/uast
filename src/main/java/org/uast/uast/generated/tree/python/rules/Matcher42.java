...
public final class Matcher42 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher42.EXPECTED_TYPE)
            && node.getChildCount() == Matcher42.EXPECTED_COUNT
            && Matcher42.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher43.INSTANCE.match(
            node.getChild(Matcher42.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher44.INSTANCE.match(
            node.getChild(Matcher42.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher45.INSTANCE.match(
            node.getChild(Matcher42.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
