...
public final class Matcher149 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher149.EXPECTED_TYPE)
            ...
            && Matcher149.EXPECTED_DATA.equals(node.getData())
            && Matcher149.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher150.INSTANCE.match(
            node.getChild(Matcher149.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher151.INSTANCE.match(
            node.getChild(Matcher149.SECOND_CHILD_ID), children, data
        );
        return flag;
    }
}
