...
public final class Rule8 implements Converter {
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        final boolean matched = Matcher9.INSTANCE.match(node, children, data);
        if (matched) {
            result = Rule8.firstBuilder(factory, data);
        }
        return result;
    }
    ...
    private static Node firstBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule8.STRING_LITERAL);
