...
public final class Matcher23 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher23.EXPECTED_TYPE)
            ...
            && Matcher24.INSTANCE.match(node.getChild(1), children, data);
        if (result) {
            children.put(
                Matcher23.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher23.FIRST_CHILD_ID))
            );
            children.put(
                Matcher23.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher23.SECOND_CHILD_ID))
            );
        }
        return result;
    }
