...
public final class Matcher111 implements Matcher {
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher112.INSTANCE.match(
            node.getChild(Matcher111.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher113.INSTANCE.match(
            node.getChild(Matcher111.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher114.INSTANCE.match(
            node.getChild(Matcher111.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
