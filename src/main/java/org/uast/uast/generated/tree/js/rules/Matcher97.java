...
public final class Matcher97 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher97.EXPECTED_TYPE)
            && node.getChildCount() == Matcher97.EXPECTED_COUNT
            && Matcher97.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher98.INSTANCE.match(
            node.getChild(Matcher97.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher99.INSTANCE.match(
            node.getChild(Matcher97.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher100.INSTANCE.match(
            node.getChild(Matcher97.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
