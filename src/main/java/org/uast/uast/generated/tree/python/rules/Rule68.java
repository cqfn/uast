...
public final class Rule68 implements Converter {
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        final boolean matched = Matcher243.INSTANCE.match(node, children, data);
        if (matched) {
            result = Rule68.firstBuilder(factory, children, data);
        }
        return result;
    }
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule68.CLASS_DECLARATIO);
        final List<Node> list = new LinkedList<>();
        list.add(Rule68.secondBuilder(factory, data));
        list.add(Rule68.thirdBuilder(factory, children));
        list.add(Rule68.fifthBuilder(factory));
    ...
    private static Node secondBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule68.IDENTIFIER);
    ...
    private static Node thirdBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule68.EXTENDS_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule68.fourthBuilder(factory, children));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node fourthBuilder(final Factory factory,
        final Map<Integer, List<Node>> children) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule68.CLASS_TYPE);
    ...
    private static Node fifthBuilder(final Factory factory) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule68.CLASS_BODY);
        if (builder.isValid()) {
            result = builder.createNode();
        }
        return result;
