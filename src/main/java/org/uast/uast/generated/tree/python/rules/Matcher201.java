...
public final class Matcher201 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        return node.belongsToGroup(Matcher201.EXPECTED_TYPE)
            && node.getChildCount() == Matcher201.EXPECTED_COUNT
            && Matcher201.matchChildren(node, children, data);
    }
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher202.INSTANCE.match(
            node.getChild(Matcher201.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher203.INSTANCE.match(
            node.getChild(Matcher201.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher205.INSTANCE.match(
            node.getChild(Matcher201.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
