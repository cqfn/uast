...
public final class Matcher33 implements Matcher {
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher34.INSTANCE.match(
            node.getChild(Matcher33.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher35.INSTANCE.match(
            node.getChild(Matcher33.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher36.INSTANCE.match(
            node.getChild(Matcher33.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
