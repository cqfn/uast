 ...
 * @since 1.0
 */
public final class Rule143 implements Converter {
    /**
     * The instance.
     */
    public static final Converter INSTANCE = new Rule143();
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        final boolean matched = Matcher251.INSTANCE.match(node, children, data);
        if (matched) {
    ...
    private static Node firstBuilder(final Factory factory,
        ...
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule143.FUNCTION_DECLARA);
        final List<Node> list = new LinkedList<>();
        list.add(Rule143.secondBuilder(factory, data));
        list.addAll(children.get(Rule143.FIRST_HOLE_ID));
        list.addAll(children.get(Rule143.SECOND_HOLE_ID));
        list.add(Rule143.fifthBuilder(factory, children));
    ...
    private static Node secondBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule143.MODIFIER_BLOCK);
        final List<Node> list = new LinkedList<>();
        list.add(Rule143.thirdBuilder(factory, data));
        list.add(Rule143.fourthBuilder(factory, data));
        final boolean applied = builder.setChildrenList(list);
        if (applied && builder.isValid()) {
            result = builder.createNode();
        }
    ...
    private static Node thirdBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule143.MODIFIER);
