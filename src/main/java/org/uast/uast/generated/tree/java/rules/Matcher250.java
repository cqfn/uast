...
public final class Matcher250 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        if (result) {
            children.put(
                Matcher250.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher250.FIRST_CHILD_ID))
            );
            children.put(
                Matcher250.SECOND_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher250.SECOND_CHILD_ID))
            );
            children.put(
                Matcher250.THIRD_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher250.THIRD_CHILD_ID))
            );
            children.put(
                Matcher250.FOURTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher250.FOURTH_CHILD_ID))
            );
            children.put(
                Matcher250.FIFTH_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher250.FIFTH_CHILD_ID))
            );
        }
        return result;
    }
