...
public final class Rule59 implements Converter {
    ...
    public Node convert(final Node node, final Factory factory) {
        ...
        if (matched) {
            result = Rule59.firstBuilder(factory, data);
        }
        return result;
    }
    ...
    private static Node firstBuilder(final Factory factory, final Map<Integer, String> data) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule59.CLASS_DECLARATIO);
    ...
    private static Node thirdBuilder(final Factory factory) {
        Node result = EmptyTree.INSTANCE;
        final Builder builder = factory.createBuilder(Rule59.CLASS_BODY);
        if (builder.isValid()) {
            result = builder.createNode();
        }
        return result;
