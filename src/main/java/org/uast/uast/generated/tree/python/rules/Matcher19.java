...
public final class Matcher19 implements Matcher {
    ...
    private static boolean matchChildren(final Node node,
        final Map<Integer, List<Node>> children,
        final Map<Integer, String> data) {
        boolean flag = Matcher20.INSTANCE.match(
            node.getChild(Matcher19.FIRST_CHILD_ID), children, data
        );
        flag = flag && Matcher21.INSTANCE.match(
            node.getChild(Matcher19.SECOND_CHILD_ID), children, data
        );
        flag = flag && Matcher22.INSTANCE.match(
            node.getChild(Matcher19.THIRD_CHILD_ID), children, data
        );
        return flag;
    }
}
