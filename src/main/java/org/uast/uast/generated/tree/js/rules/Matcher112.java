...
public final class Matcher112 implements Matcher {
    ...
    public boolean match(final Node node,
        ...
        final boolean result = node.belongsToGroup(Matcher112.EXPECTED_TYPE)
            && node.getChildCount() == Matcher112.EXPECTED_COUNT;
        if (result) {
            children.put(
                Matcher112.FIRST_HOLE_ID,
                Collections.singletonList(node.getChild(Matcher112.FIRST_CHILD_ID))
            );
        }
        return result;
    }
